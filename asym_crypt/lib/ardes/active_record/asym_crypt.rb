...
module Ardes
  module ActiveRecord#:nodoc:
    ...
    #
    # === Options
    # * <tt>:encryption_key</tt>: use specified key (an Ardes::AysmCrypt::Key) to encrypt this attribute
    # * <tt>:decryption_key</tt>: use specified key (an Ardes::AsymCrypt::Key) to decrypt this attribute
    ...
    #
    #   Ardes::ActiveRecord::AsymCrypt.encryption_key = (your key)
    #   Ardes::ActiveRecord::AsymCrypt.decryption_key = (your key)
    #
    ...
    #
    #   Secret.encryption_key = Ardes::AsymCrypt.key_from_file('my/key/location.pub')
    #                                           # set public key for encryption on class
    #
    #   @secret.secret_encryption_key           # => the Ardes::AsymCrypt::Key above
    ...
    #   @secret.secret_decryption_key =  Ardes::AsymCrypt.key_from_file('my/key/location')
    ...
    module AsymCrypt
      ...
      def asym_encrypt(attr_name, options = {})
        append_features_to_active_record unless self.included_modules.include? Ardes::ActiveRecord::AsymCrypt::InstanceMethods
    ...
    protected
      ...
      protected
        def get_key(type, attr_name = nil)
          ...
          Ardes::ActiveRecord::AsymCrypt.send "#{type}_key" 
        end

        def encrypt_attribute(attr_name, plain)  
          key = get_key(:encryption, attr_name)
          raise EncryptionKeyRequired unless key.is_a?(Ardes::AsymCrypt::Key)
        ...
        def decrypt_attribute(attr_name, cryptext)
          ...
          raise DecryptionKeyRequired unless (key = get_key(:decryption, attr_name)).is_a?(Ardes::AsymCrypt::Key)
