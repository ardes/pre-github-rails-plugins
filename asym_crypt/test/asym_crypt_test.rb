...
context "Ardes::AsymCrypt" do
  
  specify "can create key pairs of various sizes" do
    (priv, pub) = Ardes::AsymCrypt.create_keys
    assert_kind_of Ardes::AsymCrypt::Key, priv
    assert_equal :private, priv.type
    assert_kind_of Ardes::AsymCrypt::Key, pub
    assert_equal :public, pub.type
    (priv, pub) = Ardes::AsymCrypt.create_keys(1024)
    assert_kind_of Ardes::AsymCrypt::Key, priv
    assert_equal :private, priv.type
    assert_kind_of Ardes::AsymCrypt::Key, pub
    assert_equal :public, pub.type
  ...
  specify "can create key files" do
    (priv, pub) = Ardes::AsymCrypt.create_key_files(File.dirname(__FILE__) + '/fixtures/keys/test')
  ...
  specify "can instantiate public key from file" do
    key = Ardes::AsymCrypt::Key.from_file(File.dirname(__FILE__) + '/fixtures/keys/key.pub')
    assert_kind_of Ardes::AsymCrypt::Key, key
    assert_equal :public, key.type
  end
  
  specify "can instantiate public key from text" do
    key = Ardes::AsymCrypt::Key.new(
<<-end_key
...
end_key
  ...
  specify "can instantiate private key from file" do
    key = Ardes::AsymCrypt::Key.from_file(File.dirname(__FILE__) + '/fixtures/keys/key')
    assert_kind_of Ardes::AsymCrypt::Key, key
...
end_key
  ...
  specify "can instantiate key pair from files" do
    (priv, pub) = Ardes::AsymCrypt::keys_from_file(File.dirname(__FILE__) + '/fixtures/keys/key')
    assert_kind_of Ardes::AsymCrypt::Key, priv
    assert_equal :private, priv.type
    assert_kind_of Ardes::AsymCrypt::Key, pub
    assert_equal :public, pub.type
