...
describe CachedDbFile, " (new, with no attributes)" do
  before do
    @cache = CachedDbFile.new
  end
...
describe CachedDbFile, " (new, with :filename set)" do
  before do
    @cache = CachedDbFile.new(:filename => 'foo.txt')
  end
...
describe CachedDbFile, ' (saved)' do
  before(:all) do
  ...
  it 'should have #full_filename == "#{cached_db_file_root}/#{plural_model_name}/#{id_path}/#{timestamped_filename}"' do
    @cache.full_filename.should == "#{@cache.cached_db_file_root}/cached_db_files/#{@cache.id_path}/#{@cache.send(:timestamped_filename)}"
  end
  
  it 'should have #public_filename == "/#{plural_model_name}/#{id_path}/first_#{timestamp}.txt"' do
    @cache.public_filename.should == "/cached_db_files/#{@cache.id_path}/#{@cache.send(:timestamped_filename)}"
  end
end

describe CachedDbFile, " (saved, but when there is NOT a cache file)" do
  before(:all) do
